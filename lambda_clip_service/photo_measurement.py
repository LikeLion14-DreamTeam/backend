"""
사진 축별 실측값(0~100) + CLIP 임베딩 계산 함수 — Lambda 컨테이너(clip-scoring)용 사본.

원본은 `taste/photo_measurement.py`. 두 파일은 내용이 동일해야 한다 — 로직을 고치면 반드시
양쪽 다 수정할 것(2026-08-16 결정, docs/IMPLEMENTATION.md 참고).

taste 온보딩 카탈로그는 여전히 사전계산 결과(`taste/photo_measurements.py`)만 읽어서 이 모듈이
필요 없지만, recommendations 앱은 유저가 매번 새로 올리는 사진을 런타임에 스코어링해야 해서
이 모듈(과 CLIP/torch)이 실제로 필요하다 — 그래서 별도 컨테이너(Lambda)로 분리했다.
