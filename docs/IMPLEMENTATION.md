...
#### 2026-08-19 — `order` 부여 기준 수정: 선정 우선순위 ≠ 표시 순서
- **버그**: 도시에 핀이 4개 이상이라 우선순위 캡(3개)이 걸리면, `_select_pins_for_city`가
  반환하는 순서(1순위→2순위→3순위 티어 순)를 그대로 `PhotobookPin.order`로 써서, 실제 방문
  시각(`tagged_at`) 순서와 다르게 표시됨 — 포토북 지도 동선/카드 순서가 앱 내 실제 지도(NFC
  태깅 순)와 어긋나는 문제로 발견.
- **수정**: 어떤 핀을 고르는지(우선순위 규칙)는 그대로 두고, 고른 핀들에 `order`를 부여하기
  직전에 `tagged_at` 기준으로 재정렬. `build_photobook_pins`/`recompute_photobook_city` 둘 다
  동일하게 적용.
- **영향 범위**: `photobooks/services.py`만 수정. 기존에 이미 생성된 포토북의 `PhotobookPin.order`
  는 소급 반영되지 않음(재계산 스크립트 없음 — 필요 시 별도 요청).
...
### 2026-08-16 — 5.5에 5.2.1 최초 추천 연결 (#75)
- **결정**: `travel/_pin_photos_register`(5.5)에서 사진 추가 후, 그 핀에 `taste_rank`가 채워진
  사진이 하나도 없을 때만(= 한 번도 스코어링된 적 없는 핀) `rank_pin_photos(pin)`을 호출한다.
  이미 스코어링된 핀에 사진이 추가로 등록되는 경우엔 자동 재계산하지 않는다.
- **이유**: 명세서엔 "핀 생성 시점에 스코어링"이라고만 돼 있는데, 실제로는 핀 생성(`POST /pins`)과
  사진 등록(`POST /pins/{pinId}/photos`)이 분리된 별도 API라 핀 생성 시점엔 사진이 없다. "언제가
  최초 스코어링 시점인가"를 코드로 판별 가능한 조건(기존 `taste_rank` 유무)으로 확정했다 — 별도
  플래그 필드를 추가하지 않기 위함.
- **영향 범위**: `travel/views.py`(`_pin_photos_register`), `travel/tests.py`(신규, 이전엔 빈 파일).

### 2026-08-16 — 5.6 대표사진 새로고침 엔드포인트 신설 (#81)
- **결정**: `POST /pins/{pinId}/representative-photos/refresh` 신규 구현. 핀에 연결된 사진이
  4장 미만이면 `409 CONFLICT`로 거부한다.
- **이유**: 명세서엔 "3장 이하면 재추천 버튼 없음"이라고만 돼 있어 FE 버튼 숨김만으로 해석될 수
  있었으나, API 직접 호출까지 막으려면 백엔드 검증이 필요하다고 판단 — `trip_create`의 "종료할
  여행 없음" 409 등 기존 엔드포인트들의 서버측 검증 패턴과 일관성 유지.
- **영향 범위**: `travel/urls.py`, `travel/views.py`(`pin_photos_refresh` 신규), `travel/tests.py`,
  `docs/Orte_API_명세서.md`(5.6에 409 응답 명세 추가 — 기존엔 성공 케이스만 있었음).

### 2026-08-16 — 6.4 포토북 커버 새로고침 엔드포인트 신설, 무작위 방식으로 단순화 (#85)
- **결정**: `POST /photobooks/{photobookId}/cover/refresh` 신규 구현. 명세서 원문의 "취향
  프로파일 기준 재정렬 → 상위 10개 → 무작위 1개" 대신, **취향 스코어링 없이 포토북에 이미
  포함된 사진(`PhotobookPhotoLayout`) 중 순수 무작위 1장**으로 단순화. `recommendations` 앱은
  이 작업에서 전혀 사용하지 않는다.
- **이유**: 원문대로면 새로고침을 누를 때마다 해당 여정 전체 사진(짧은 여행도 수십 장 이상일 수
  있음)을 매번 다시 CLIP으로 스코어링해야 해서 비용이 큼. 반면 5.6(핀 하나 단위)은 후보가 적어
  이 문제가 크지 않았음. 커버 사진은 "가장 취향에 맞는 1장"보다 "다양하게 보여주는" 목적이 더
  크다고 판단해 스코어링 자체를 생략하기로 결정. 후보 범위도 여정 전체가 아니라 포토북에 이미
  큐레이션된 사진(도시당 최대 3핀 × 핀당 최대 4장)으로 한정해 범위를 예측 가능하게 유지.
- **영향 범위**: `photobooks/urls.py`, `photobooks/views.py`(`photobook_cover_refresh` 신규),
...
### 2026-08-19 — 사진 등록 요청당 15장 제한 및 등록 시점 실측값 eager 캐싱 (#110)
- **결정**: `POST /pins/{pinId}/photos` 요청당 최대 15장으로 제한, 초과분은 기존
  `PHOTO_LIMIT_EXCEEDED`와 동일한 부분 거절 방식으로 `BATCH_SIZE_EXCEEDED` 반환(체크 순서:
  15장 배치 제한 → 50장 핀 총량 제한). 이미 스코어링된 핀(`taste_rank`가 채워진 사진이
  하나라도 있는 핀)에 사진이 추가될 때는 `taste_rank`는 건드리지 않되(재계산은 5.6
  새로고침에서만), 새로 추가된 사진의 실측값(`measured_*`, #104 캐시 필드)만 등록 시점에
  미리 계산해 캐싱한다(`precompute_measured_axes`).
- **이유**: #104 캐싱 이후에도, 핀에 미분석 사진이 누적된 채로 재추천을 요청하면 그 순간
  누적분 전체가 한 요청 안에서 계산되어 클라이언트 타임아웃(30초) 위험이 있었다. 등록
  시점에 15장 단위로 나눠 미리 계산해두면 이 누적 계산이 사라진다. 프론트 API 계약(요청/
  응답 스키마)은 그대로 유지하면서(비동기 큐 도입 없이) 해결할 수 있는 절충안으로 채택.
- **영향 범위**: `travel/views.py`(`_pin_photos_register`, `MAX_PHOTOS_PER_REQUEST`),
  `recommendations/travel_adapter.py`(`precompute_measured_axes`), `travel/tests.py`.
- **미확정(알려진 한계)**: `already_scored` 판단 기준이 "이 핀에 `taste_rank`가 채워진
  사진이 하나라도 있는가"라서, **온보딩(취향 프로파일)을 완료하기 전에** 사진을 먼저
  여러 번 나눠 올린 경우(그동안은 매번 스코어링이 no-op으로 스킵되어 `measured_*`도 안
  채워짐) **그 다음에 온보딩을 완료하고** 그 핀에 사진을 추가하면, `rank_pin_photos`가
  누적된 사진 전체(최대 50장)를 그 한 요청 안에서 몰아서 계산한다 — 이번 15장 제한은
  "이미 스코어링된 핀"에만 적용되는 안전장치라 이 케이스는 보호 대상이 아니다. 온보딩을
  먼저 끝내고 사진을 올리는 정상 플로우에서는 발생하지 않는다(첫 등록 시점에 바로
  스코어링되어 `already_scored`가 곧바로 True로 바뀌므로). 발생 빈도가 낮아 이번 작업
  범위에서는 제외, 필요해지면 별도 이슈로 다룬다.
