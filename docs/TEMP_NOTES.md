...
### [x] recommendations 앱 CLIP 런타임 배포 (미착수) — CLIP 자체를 제거해 해소
- **임시로 한 것**: taste 온보딩 카탈로그는 정적 사전계산으로 CLIP 배포 부담을 없앴지만,
  recommendations(5.2.1, 유저 업로드 사진 스코어링)는 사진이 매번 새로 올라와서 런타임에 CLIP을
  실제로 돌려야 함 — recommendations 앱 착수 후에도 배포 방식(Docker 이미지 포함 vs 영구
  볼륨 캐싱)이 한동안 미정 상태였음.
- **정식으로 교체한 것**: 배포 방식을 정하는 대신, CLIP 의존성 자체를 완전히 제거했다
  (유사사진 판별→pHash, 구도/밀도 축→블러+엣지검출). "CLIP을 어떻게 배포할까"라는 질문이
  전제 자체가 없어짐. 자세한 내용은 `docs/IMPLEMENTATION.md` 2026-08-16 결정 로그(#94) 참고.
- **완료**: #94, 2026-08-16
