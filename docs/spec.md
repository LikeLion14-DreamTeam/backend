...
| 6.4 포토북 커버 새로고침 선정 로직 | API 명세서: "취향 프로파일 기준 재정렬 → 상위 10개 → 무작위 1개" | **취향 스코어링 없이, 포토북에 이미 포함된 사진(`PhotobookPhotoLayout`) 중 순수 무작위 1장으로 단순화.** 새로고침마다 여정 전체 사진을 다시 스코어링하면 CLIP 연산 비용이 커서, 대표성보다 "다양하게 보여주기" 목적에 맞춰 결정. 상세 이유는 `docs/IMPLEMENTATION.md` 참고 | ✅ 반영 완료 (2026-08-16, #85) |
...
- ~~미확정: 재추천 시 후보 사진이 10장 미만인 핀의 처리 기준~~ **[정정]** 이미 확정됨 — 10장
  미만이면 등록된 사진 전체를 후보로 사용, 정확히 3장이면 무작위 없이 그대로 사용(위 체크리스트
  "재추천 시 후보 10장 미만 처리" 행, 옵션 A로 확정). 이 문단이 나중에 추가돼 체크리스트와
  모순돼 있었음 — 체크리스트가 최신.
