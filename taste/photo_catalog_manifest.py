...
#
# A/B(round_no 1~5)의 `value`는 실제 유저 응답이 아니라, 기획 단계에서 "이 사진은 축의 20/80
# 쪽으로 의도했다"고 정한 참조값이다. 측정 함수(HSV/CLIP 등) 구현 후 이 참조값과 실제 측정치가
# 같은 방향으로 나오는지 검증하는 용도로만 쓴다.
# 무드보드(round_no 6~7)는 사전 참조값이 없다 — 측정 함수가 계산한 값만 사용한다.
